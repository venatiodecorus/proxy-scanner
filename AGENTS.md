...
1. **Scanner** (`cmd/scanner/`, `docker/Dockerfile.scanner`) — Go binary wrapping masscan. Runs on-demand via Docker Compose scan profile. Scans IPv4 space on the SOCKS ports `1080,1081,4145,9050`, then enqueues candidates into the SQLite database. The Scanner writes masscan JSON output to disk (for debugging) but the primary data path is the `candidates` queue table in SQLite.
...
- **Protocols and ports**: New scans and validation are SOCKS4/SOCKS5 only. The default target ports are `1080,1081,4145,9050`.
...
- **Scan resume**: The scanner supports masscan's `--resume` feature for incremental scanning. When `SCAN_TIMEOUT` is set, the scanner sends SIGINT to masscan after the timeout, causing masscan to save its state to `/data/paused.conf`. On the next run, the scanner detects this file and resumes from where it left off, so successive sessions make progress through the IPv4 space without re-scanning covered ranges.
- **Never pass a target range alongside `--resume`.** masscan unions every target range it is given, and this build stores progress in `paused.conf` as the list of *remaining* ranges (there is no `resume-index`). A bare `0.0.0.0/0` on a resumed invocation restores the already-scanned space, so every session rescans the same opening slice and the sweep never advances. `buildMasscanArgs` emits exactly one of `--resume` or a target; `TestBuildMasscanArgsResumeOmitsTarget` and `TestBuildMasscanArgsTargetIsExclusive` guard it. Symptom of a regression: a resumed run logging the full in-scope host count instead of a smaller remainder.
- **`--excludefile` goes on every invocation, resume included.** masscan does not persist excludes into `paused.conf` (upstream #110) and refuses a large scan without at least one exclude.
- **The scanner deletes `paused.conf` when a sweep completes**, so the next session starts a fresh pass. masscan writes that file only on interrupt and never clears a stale one, so leaving it would make the next session resume a finished sweep and rescan its tail forever.
...
- `SCAN_PORTS` — Comma-separated SOCKS port list (default: `1080,1081,4145,9050`)
...
- **Schedule** is systemd timers, defined in `deploy/systemd/` and installed to `/etc/systemd/system/` by `deploy/bootstrap.sh`: `proxy-scanner-scan.timer` every eight hours at 02:00, 10:00, and 18:00 UTC, and `proxy-scanner-validate.timer` every eight hours at 07:00, 15:00, and 23:00 UTC. The validator also chains off the scanner via `OnSuccess=` so the queue drains as soon as a sweep session ends. Full runbook in `deploy/README.md`.
...
- A full IPv4 sweep is ~75h of masscan time (3.37B addresses in scope × 4 ports / 50k pps), so three daily 4h sessions complete a sweep about every 6.2 days. Recompute if `SCAN_PORTS`, `SCAN_RATE`, or the exclusion lists change — some older notes in this repo assumed a single port and understated it as ~24h.
